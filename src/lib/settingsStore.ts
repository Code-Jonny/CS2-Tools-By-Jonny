...
import { storage as neutralinoJsStorageAPI } from "@neutralinojs/lib";
...
export const defaultAppSettings = {
  ...
  // Add other settings with their default values here, e.g.:
  // theme: 'dark',
...
const settingsProxyHandler: ProxyHandler<AppSettings> = {
  set: (target, propertyKey: string, value, receiver) => {
  ...
  // Define other traps like deleteProperty if needed, though likely not for this use case.
};

// Export the proxy. Components will interact with this.
export const settings = new Proxy(_internalSettings, settingsProxyHandler);
...
export async function resetToDefaults() {
  ...
  try {
    for (const key in defaultAppSettings) {
      ...
      try {
        await neutralinoJsStorageAPI.removeData(typedKey);
      } catch (e) {
        console.warn(`Could not remove key ${typedKey} during reset:`, e);
      }

      // Update the reactive _internalSettings object with defaults
      const defaultValue = defaultAppSettings[typedKey];
      _internalSettings[typedKey] = defaultValue; // Update reactive state

      // Explicitly save each default setting back to storage
